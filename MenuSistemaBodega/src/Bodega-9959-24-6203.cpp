...
void Nominas::cargarEmpleados() {
    ...
    ifstream archivo("bodega.dat", ios::binary);
...
void Nominas::guardarEmpleados() {
    try {
        ofstream archivo("bodega.dat", ios::binary | ios::trunc);
        if (!archivo.is_open()) {
...
void Nominas::menuNominas() {
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 4: desplegarEmpleados(); break;
...
void Nominas::crearEmpleado() {
    ...
    bitacoralogNominas.insertar(usuario, 4001, "Bodega", "Crear empleado");
...
void Nominas::borrarEmpleado() {
    ...
    if (eliminado) {
        ...
        guardarEmpleados();
        bitacoralogNominas.insertar(usuario, 4002, "Bodega", "Eliminar empleado");
...
void Nominas::buscarEmpleado() {
    ...
    for (const auto& e : empleados) {
        if (e.nombre == nombre && e.codigo == codigo) {
            ...
            encontrado = true;
            bitacoralogNominas.insertar(usuario, 4003, "Bodega", "Buscar empleado");
            break;
        }
    }
...
void Nominas::modificarEmpleado() {
    ...
    for (auto& e : empleados) {
        if (e.nombre == nombre && e.codigo == codigo) {
            ...
            cout << "Nuevo Estado: "; getline(cin, e.telefono);
    ...
    if (modificado) {
        ordenarEmpleados();
        guardarEmpleados();
        bitacoralogNominas.insertar(usuario, 4004, "Bodega", "Modificar empleado");
...
void Nominas::desplegarEmpleados() {
    ...
    if (empleados.empty()) {
        ...
        cout << "\n-----------------------------";
        bitacoralogNominas.insertar(usuario, 4005, "Bodega", "Ver empleados");
    }
